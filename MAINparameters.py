...
class Parameters(object):
    ...
    def __init__(self, name_exp, feature_role, iterations, type):
        ...
        self.PATH_PETRINET = self.NAME_EXP + '/' + type + '/' + self.NAME_EXP + '.pnml'
        if self.FEATURE_ROLE == 'all_role':
            self.prefix = ('_diapr', '_dpiapr', '_dwiapr')
        else:
            self.prefix = ('_dispr', '_dpispr', '_dwispr')

        self.MODEL_PATH_PROCESSING = self.NAME_EXP + '/' + type + '/' + self.NAME_EXP + self.prefix[1] +'.h5'
        self.MODEL_PATH_WAITING = self.NAME_EXP + '/' + type + '/' + self.NAME_EXP + self.prefix[2] + '.h5'
        ...
        self.METADATA = self.NAME_EXP + '/' + type + '/' + self.NAME_EXP + self.prefix[0] + '_meta.json'
        self.SCALER = self.NAME_EXP + '/' + type + '/' + self.NAME_EXP + self.prefix[0] + '_scaler.pkl'
        self.INTER_SCALER = self.NAME_EXP + '/' + type + '/' + self.NAME_EXP + self.prefix[0] + '_inter_scaler.pkl'
        self.END_INTER_SCALER = self.NAME_EXP + '/' + type + '/' + self.NAME_EXP + self.prefix[0] + '_end_inter_scaler.pkl'
